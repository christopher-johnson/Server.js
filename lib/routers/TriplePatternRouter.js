...
var propertyPathMatcher = /^([a-z0-9\-]*):([a-z0-9\-]+)(\*)$/i;
...
TriplePatternRouter.prototype.extractQueryParams = function (request, query) {
  ...
  if (queryString.predicate) {
    // the predicate can be a propertyPath (https://www.w3.org/TR/sparql11-property-paths)
    if (match = propertyPathMatcher.exec(queryString.predicate))
      hasTriplePattern = query.predicate = match[0];
    // or the object can be an IRI…
    else if (match = iriMatcher.exec(queryString.predicate))
